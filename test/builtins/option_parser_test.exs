defmodule RShell.Builtins.OptionParserTest do
  ...
  describe "parse/2 with boolean options" do
    ...
    test "parses long boolean flag" do
      specs = [
        %{long: "--no-newline", type: :boolean, default: false, key: :no_newline, description: "No newline"}
      ]

      assert {:ok, %{no_newline: true}, ["hello"]} = OptionParser.parse(["--no-newline", "hello"], specs)
    end
    ...
    test "handles multiple flags" do
      specs = [
        %{short: "-n", type: :boolean, default: false, key: :no_newline, description: "No newline"},
        %{short: "-e", type: :boolean, default: false, key: :enable_escapes, description: "Enable escapes"}
      ]
  ...
  describe "parse/2 with integer options" do
    ...
    test "returns error for invalid integer" do
      ...
      assert {:error, "Option -c requires an integer value"} = OptionParser.parse(["-c", "abc"], specs)
    end
  end
