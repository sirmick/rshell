defmodule BashParser.AST.Types do
  ...
  This file is auto-generated by `mix gen.ast_types`.

  Manual changes will be OVERWRITTEN when the generator runs.

  If you need to modify AST types, edit the generator at:
  lib/mix/tasks/gen/ast_types.ex

  Exception: The ErrorNode module (lines 622-649) was manually added
  because ERROR is not part of the tree-sitter grammar, but is created
  during error recovery. This module should be preserved if regenerating.
  """
  ...
  defmodule SourceInfo do
    ...
    Includes tree-sitter node metadata flags for error recovery:
    - is_missing: Node is expected but not present (parser anticipates it) - NOTE: tree-sitter-bash never sets this
    - is_extra: Node is extra (not part of grammar but can appear anywhere, e.g., comments)
    - is_error: Node IS an ERROR node (content doesn't match grammar)
    - has_error: Node CONTAINS an error or missing node somewhere inside it
    """
  ...
  defmodule ErrorNode do
    ...
    ⚠️  MANUALLY ADDED - DO NOT REMOVE ⚠️

    Special node type created by tree-sitter when it encounters syntax errors.
    This module was manually added because ERROR is not part of the grammar
    but is created during error recovery.

    These nodes indicate actual syntax problems, not incomplete structures.
    Tree-sitter creates ERROR nodes for both:
    1. Syntax errors (e.g., malformed arithmetic expressions)
    2. Incomplete structures (e.g., 'for i in 1 2 3' without 'do')

    Note: Tree-sitter does NOT reliably set is_missing flags for incomplete
    structures. Instead, it creates ERROR nodes. Use the presence of ERROR
    nodes to detect incompleteness or syntax issues.
    """
    @enforce_keys [:source_info]
