defmodule Mix.Tasks.Gen.RshellAstTypes do
  @moduledoc """
  Generates typed Elixir structs from tree-sitter-rshell node-types.json schema.

  Outputs all types to a single file: lib/bash_parser/ast/rshell_types.ex
  ...
  defp determine_category(%{"type" => type}) do
    cond do
      # RShell-specific syntax
      type in [
        "rshell_assignment",
        "rshell_expression",
        "rshell_binary_expression",
        "list_literal",
        "map_literal",
        "map_entry",
        "boolean_literal"
  ...
  defp print_statistics(categorized) do
    ...
    Enum.each(categorized, fn {category, nodes} ->
      ...
      if count > 0 do
        Mix.shell().info(
          "  #{category |> to_string() |> String.pad_trailing(16)}: #{count} types"
        )
      end
    end)

    total = categorized |> Map.values() |> Enum.map(&length/1) |> Enum.sum()
    Mix.shell().info("  #{"Total" |> String.pad_trailing(16)}: #{total} types")
  end
  ...
  defp generate_types_file(categorized, output_path) do
    ...
    defmodule BashParser.AST.RShellTypes do
      ...
      Auto-generated from tree-sitter-rshell grammar (#{length(all_nodes)} node types).
  ...
  defp generate_node_module(node) do
    ...
    """
      defmodule #{module_name} do
        ...
        @type t :: %__MODULE__{
                source_info: BashParser.AST.RShellTypes.SourceInfo.t()#{if fields != [] or has_unnamed_children, do: ",", else: ""}
    #{generate_field_type_specs(fields)}#{if has_unnamed_children and fields != [], do: ",\n", else: ""}#{if has_unnamed_children, do: "          children: list(any())", else: ""}
        ...
        def from_map(data) do
          %__MODULE__{
            source_info: BashParser.AST.RShellTypes.SourceInfo.from_map(data)#{if fields != [] or has_unnamed_children, do: ",", else: ""}
    #{generate_field_from_map(fields)}#{if has_unnamed_children and fields != [], do: ",\n", else: ""}#{if has_unnamed_children, do: "          children: BashParser.AST.RShellTypes.extract_children(data, \"children\")", else: ""}
          }
        end
    ...
    |> Enum.map(fn field ->
      value =
        if field.multiple do
          "BashParser.AST.RShellTypes.extract_children(data, \"#{field.name}\")"
        else
          "BashParser.AST.RShellTypes.extract_field(data, \"#{field.name}\")"
        end
